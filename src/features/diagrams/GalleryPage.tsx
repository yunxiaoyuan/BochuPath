...
export function GalleryPage({ theme, onTheme }: Props) {
  ...
  useEffect(() => {
    let active = true;
    setItems([]);
    const reload = () => void getRepository().list().then(next => { if (active) setItems(next); }).catch(error => { if (active) setMessage(error instanceof ApiError ? error.message : '图库加载失败'); });
    reload();
    window.addEventListener('focus', reload);
    return () => { active = false; window.removeEventListener('focus', reload); };
  }, [session?.user.userId]);
  const openCreate = () => { if (!canWrite) return; setNewName('未命名通路图'); setCreateOpen(true); };
  const create = async () => { if (!canWrite) return; const name = newName.trim(); if (!name) { setMessage('请输入通路图名称'); return; } try { const diagram = await getRepository().create({ name }); setCreateOpen(false); navigate(`/diagrams/${diagram.id}/edit`); } catch (error) { setMessage(error instanceof ApiError ? error.message : '新建失败，请检查共享数据连接或浏览器存储'); } };
  const duplicate = async (item: DiagramSummary) => { const name = (await dialog.prompt({ title: '复制通路图', label: '副本名称', defaultValue: `${item.name} 副本`, confirmLabel: '复制' }))?.trim(); if (!name) return; try { await getRepository().duplicate(item.id, name); await refresh(); setMessage('复制成功'); } catch (error) { setMessage(error instanceof ApiError ? error.message : '复制失败'); } };
  const remove = async (item: DiagramSummary) => { if (!await dialog.confirm({ title: '删除通路图', message: `删除“${item.name}”？此操作不可撤销。`, confirmLabel: '删除', destructive: true })) return; try { await getRepository().delete(item.id); await refresh(); setMessage('已删除通路图'); } catch (error) { setMessage(error instanceof ApiError ? error.message : '删除失败；图库至少需要保留一张图'); } };
  const rename = async (item: DiagramSummary) => { const name = (await dialog.prompt({ title: '重命名通路图', label: '新名称', defaultValue: item.name, confirmLabel: '重命名' }))?.trim(); if (!name || name === item.name) return; try { const diagram = await getRepository().get(item.id); await getRepository().save(renameDiagram(diagram, { name, description: diagram.description }), item.revision); await refresh(); setMessage('重命名成功'); } catch (error) { setMessage(error instanceof ApiError ? error.message : '重命名失败'); } };
  ...
  return <div className="gallery-shell">
    <header className="gallery-header" aria-label="图库顶部栏">
      ...
      <div className="header-actions"><button className="icon-button" onClick={onTheme} aria-label={`切换到${theme === 'light' ? '深色' : '浅色'}主题`} title="切换主题">{theme === 'light' ? '◐' : '○'}</button>{canWrite && <><button onClick={() => importInput.current?.click()} disabled={importing}>{importing ? '导入中…' : '导入 JSON'}</button><input ref={importInput} className="sr-only" type="file" accept="application/json,.json" aria-label="选择要导入的 JSON 文件" onChange={(event) => void importFile(event)} /><button className="primary-button" onClick={openCreate}>＋ 新建通路图</button></>}<AccountMenu /></div>
    </header>
    <main className="gallery-main">
      <PermissionBanner />
      <section className="gallery-title"><div><p className="eyebrow">{secure ? '企业工作空间' : pageDrop ? 'PAGEDROP 异步协作' : shared ? '本机共享 JSON' : '本机工作空间'}</p><h1>通路图库</h1><p>{secure ? '查看和梳理业务通路，获得编辑权限后即可共同维护。' : shared ? '保存后写入共享数据；其他协作者刷新或重新聚焦页面即可读取最新版本。' : '创建、维护并重新打开结构化业务通路图。'}</p></div><label className="search-field"><span className="sr-only">搜索通路图</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索通路图" /></label></section>
      ...
      <section className="diagram-grid" aria-label="通路图列表">
        {canWrite && <button className="new-diagram-card" onClick={openCreate}><span>＋</span><strong>从空白图开始</strong><small>自动创建默认节点样式</small></button>}
        {visible.map((item) => <article className="diagram-card" key={item.id}>
          <button className="diagram-preview" onClick={() => navigate(`/diagrams/${item.id}/${canWrite ? 'edit' : 'view'}`)} aria-label={`${canWrite ? '编辑' : '查看'} ${item.name}`}>
          ...
          <div className="diagram-card-body"><div><h2 title={item.name}>{item.name}</h2><p>{item.nodeCount} 个节点 · {item.pathwayCount} 条通路</p><time>{new Date(item.updatedAt).toLocaleString('zh-CN')}</time></div>
            <div className="card-actions"><button onClick={() => navigate(`/diagrams/${item.id}/view`)}>查看</button>{canWrite && <><button onClick={() => void rename(item)}>重命名</button><button onClick={() => void duplicate(item)}>复制</button><button className="danger-text" onClick={() => void remove(item)}>删除</button></>}</div>
          </div>
        </article>)}
      </section>
      {!visible.length && <div className="empty-state"><strong>{query.trim() ? '没有匹配的通路图' : '图库中还没有通路图'}</strong>{query.trim() ? <button onClick={() => setQuery('')}>清除筛选</button> : canWrite ? <button className="primary-button" onClick={openCreate}>新建第一张通路图</button> : <span>获得编辑权限后即可创建第一张通路图</span>}</div>}
    </main>
    {createOpen && canWrite && <div className="modal-backdrop" role="presentation" onMouseDown={() => setCreateOpen(false)}>
