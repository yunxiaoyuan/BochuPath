...
export function createDemoDiagram(): Diagram {
  return {
    schemaVersion: '1.3', id: DEMO_DIAGRAM_ID, name: '需求到交付示例', description: '用于验证分层、节点样式和跨层通路', revision: 1,
    layers: [
...
export function createBlankDiagram(name: string, id = newId('diagram')): Diagram {
  ...
  return {
    schemaVersion: '1.3', id, name: name.trim(), revision: 0, layers: [], nodes: [], pathways: [],
