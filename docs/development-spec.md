...
| 通路 | 新增、查看、编辑、删除、画布直接增删节点、固定顺序自动编号、显示/隐藏、聚焦高亮 |
...
| Pathway | 通路名称、节点集合、颜色、线型、备注、是否显示 | 名称 1–80 字；至少 2 个且不重复的节点；步骤自动按叶子层级与同层节点的画布顺序排列，允许同层和跨层 |
...
- 以层级树中叶子层级的深度优先视觉顺序作为第一排序键，以节点在所属叶子层级内的 `order,id` 作为第二排序键；TB/LR 共用这一固定业务顺序；
- 一条通路可包含同一层的多个节点，也可跳过中间层级；通路步骤不单独排序，而是根据画布中的固定节点顺序自动规范化；
- 多条通路可共享节点和相邻节点对；通路仍是多节点有序序列，不要求覆盖全部层级；
- 删除通路只删除 Pathway 和 PathStep，不删除节点；
- 隐藏通路仅影响渲染，不删除数据；
- 节点换层、层级移动或节点/层级排序后，所有受影响通路的步骤顺序随固定画布顺序自动更新；
- 新建通路时普通点击未选节点加入，`Shift+点击` 已选节点移除；编辑已有通路时普通点击仍保留节点选择语义，`Shift+点击` 切换节点是否属于通路；
- 增删节点会实时预览自动连线和步骤编号，但点击“确定”或 Enter 前只存在于草稿；取消或 Esc 不留下半成品；
- V1 不提供通路步骤拖拽排序，也不需要选择插入位置；画布固定顺序是唯一步骤顺序。
...
编辑/连接
  ├─ 新建：点击未选节点 → 加入草稿；Shift+点击已选节点 → 移除
  ├─ 编辑：普通点击 → 保持节点选择；Shift+点击 → 切换通路成员
  ├─ 每次增删 → 按画布固定顺序自动排列并预览连线
...
- Pathway：`通路名称* → 当前步骤摘要 → 在画布编辑节点 → 颜色 → 线型 → 备注 → 是否显示`；Inspector 只读展示序号、节点名和完整层级路径，不放置插入、替换或移除控件；
...
```text
┌─ 工具栏：[连接通路]（品牌色激活）──────────────────────────────────────────┐
│ 新建：点击加入；编辑：Shift+点击增删；Enter 完成，Esc 取消      │
├───────────────────────────────────────────────────────────────────────┤
│ [节点A ①] - - - -▶ [同层节点B ②] - - - -▶ [节点C ③]              │
│ 草稿边为品牌色虚线；已加入节点显示步骤编号；顶部提供完成和取消       │
└───────────────────────────────────────────────────────────────────────┘
```

- 进入后在工具栏下插入 36px 指引条，保持当前视口中心不跳动；
- 新建通路时，普通点击未选节点即加入，`Shift+点击` 已选节点即移除；
- 编辑已有通路时，普通点击不改变通路，仍执行节点选择；`Shift+点击` 未选节点加入、已选节点移除；
- 加入顺序不影响通路顺序；系统始终按层级与同层节点的画布顺序重排步骤，并允许同层多节点；
- 已加入节点显示步骤编号，未加入节点显示轻量候选提示；增删后草稿连线和编号实时更新；
...
- 通路至少两步、重复节点拒绝、跨层和同层多节点通过、逆向原始数据报错、步骤归一化；
- 任意点选顺序都会按画布固定顺序归一化，层级/节点调整后通路步骤自动同步；
...
- 新建通路普通点击加入、Shift+点击移除；编辑通路普通点击保持选择语义、Shift+点击切换成员；Enter 提交、Esc 清空、取消不写入；
...
2. 编辑已有通路，普通点击不改变成员，Shift+点击直接增删跨层或同层节点，步骤按画布顺序自动重排，箭头与编号同步更新，Undo/Redo 正确；
...
- 通路至少两步、无重复节点，允许同层多节点，步骤始终与画布固定顺序一致；
- 自动布局稳定，相同输入得到相同坐标；失败时不白屏；
- 树、画布、Inspector 使用唯一 Selection 并保持同步；
- 查看模式没有能改变数据的控件、快捷键或拖拽行为；
- 连接模式支持画布直接增删、新建/编辑差异化点击语义、固定顺序自动排列、Enter、Esc 和草稿/已确认视觉区分；
