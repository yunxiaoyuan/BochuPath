...
> V1.1 企业微信登录和权限不属于本文 V1.0 冻结范围；开发该能力时必须同时遵守 [`access-permission-design.md`](access-permission-design.md)，尤其是服务器端鉴权和禁止前端伪权限的要求。
...
- 编辑已有通路时，选中通路即成为独立的活动通路，无需二次入口；左侧行与画布提示条持续显示“编辑中”，`Shift+点击` 或聚焦节点后 `Shift+空格` 立即执行可撤销的增删命令；普通点击节点只查看节点属性，不退出活动通路；
...
5. 在泳道中根据 `nodeWidth/nodeMinHeight/nodeGap` 排列节点；每个节点的宽高由名称和拆解文本独立测量，同一行共用最大行槽高度但不把短节点的方框强行撑高；
   - 布局接收实际画布宽高作为派生输入，在 16:9 等宽屏和密集数据下枚举紧凑节点宽度、间距与行列容量；TB 优先保持同层单行，先按文字计算自然宽度，再按每个节点的可收缩量等比压缩；达到最小宽度仍无法容纳才稳定换行，不能修改对象顺序；
   - TB 节点自然宽度限制为 `64px–240px`，无拆解内容的紧凑高度为 `32px`；标题水平、垂直居中，接近换行边界或形成两行时字号自动缩小 `1px`、行高收紧为约 `1.15`，且通过裁切检查保证文字不越出节点边缘；换行文字或拆解信息只增加自身高度。叶子泳道按最宽一层的实际内容取宽，最大不超过实际可见画布宽度减两侧 `12px`，整图居中，禁止用无意义空白撑满屏幕；
   - LR 叶子泳道按最高一列的实际内容取高，最大不超过实际可见画布高度减上下 `12px`；各节点按自身内容独立取宽高并按列垂直居中，禁止以全图最高节点统一撑高其他节点，也禁止用空白强行撑满画布；
   - 紧凑模式最小节点间距 `4px`、最小层间距 `4px`、层标题区 `24px`、泳道内容内边距 `4px`；数值均遵循 BOCHUI Lite 4px 网格；
   - 自动换行不得修改对象 `order`；用户拖动只改变同级/同层 `order`，不持久化坐标；
6. TB 模式从上到下排列泳道，LR 模式从左到右排列；父层标题区固定，不与节点重叠；
7. 生成节点锚点后再派生并路由连线，跨层使用柔和贝塞尔曲线，箭头头部统一为紧凑的 `8×8`，箭头指向下一占用层节点；常态连线以细灰色呈现，聚焦或节点关联高亮时恢复通路颜色并增粗；未命中连线在高亮前后必须保持完全相同；
8. 计算完整包围盒并支持“适应画布”；
...
编辑/选择
  ├─ 新建通路 → 编辑/新建通路草稿
  ├─ 选中已有通路 → 设为活动通路；Shift+点击/Shift+空格增删；普通点击节点仅查看属性
  ├─ 新增或编辑对象 → 编辑/表单草稿
...
- 单选节点显示品牌蓝实底、白字、2px 深品牌描边和连接控制点；取该节点参与的全部可见通路，将这些通路的完整成员节点和派生箭头作为关联并集：关联节点临时覆盖为浅蓝底、2px 品牌蓝实线边框与外环，不与原虚线/点线边框叠加；关联箭头保留通路颜色并增粗。**高亮只允许增强命中对象；所有未命中的节点、箭头、层级、泳道和其他画布元素必须与高亮前保持完全相同的颜色、透明度、饱和度、滤镜、边框和业务样式，禁止任何置灰或弱化处理。此项为不可回退的产品硬约束。** 清除选择后恢复原样式；隐藏通路不因选择而显示；
...
- “适应画布”按实际 React Flow 可见区域计算，以完整图形包围盒居中，四周保留 `12px` 安全边距；不得按包含左右面板的页面尺寸计算；整图仍完整可见时允许自动放大至 `125%`，避免内容图因紧凑后只占屏幕一小块。“布局”使用相同包围盒，但以 `max(12px / layout.fontSize, 10px / layout.descriptionFontSize, 10px / 12px层标题)` 作为缩放下限；超大图允许超出视口并通过平移阅读，不能用无限缩小字体冒充可读布局。侧栏开合、连接指引出现或窗口缩放后必须保持用户最后选择的“适应/布局”策略。
- 顶部工具栏已有缩放能力时不渲染 React Flow 左下角重复控件；右下角导航缩略图默认关闭，由工具栏按钮显式切换，关闭状态下不得遮挡节点或截获节点操作。
...
```text
┌─ 工具栏：[新建通路]（创建草稿时品牌色激活）────────────────────────────────┐
│ 新建：点击加入、Shift+点击移除；已有通路：Shift+点击/空格增删    │
├───────────────────────────────────────────────────────────────────────┤
...
- 点击通路列表项或通路边即设为活动通路并直接进入成员编辑；左侧自动切换到通路 Tab，对应行显示“编辑中”，画布工具栏下显示通路名、节点数、快捷键和退出按钮；不提供“在画布编辑节点”按钮；
- 编辑已有通路时，`Shift+点击` 或聚焦节点后 `Shift+空格` 切换成员，每次生成一条可撤销命令；普通点击节点只切换 Inspector 到节点属性，活动通路与快捷键目标保持不变；
- 点击空白只清除普通对象选择，不清除活动通路；按 Esc、点击“退出编辑”、选择层级/样式或切换模式退出；选择另一通路则直接切换编辑目标；
...
- 候选边为品牌虚线；已确认通路支持实线、虚线和点划线（— · — ·）；已有通路的成员增删立即生效，名称、颜色、线型、备注和可见性仍通过 Inspector 的“确定/取消”提交，且属性提交不得覆盖最新成员集合。
...
- 新建通路普通点击加入、Shift+点击移除，Enter/确定只提交新建草稿；活动通路无需二次按钮即可通过 Shift+点击或 Shift+空格切换成员，普通点击节点后活动通路仍保留，Esc/退出按钮/层级或样式选择可以退出；
...
2. 选中已有通路后无需按钮即可通过 Shift+点击或 Shift+空格连续增删跨层或同层节点；普通点击节点查看属性后左侧“编辑中”和画布活动通路条仍保留，Esc 退出；每次即时命令的 Undo/Redo 正确，属性表单提交不覆盖成员；
3. 两个相邻占用层各含两个节点时生成四条边；删除中间占用层后前后层自动全连接，删除会导致单层通路时被阻止；
4. 删除被引用样式前强制选择替代样式；
5. 编辑及查看模式单选节点或聚焦通路时，完整关联节点/箭头高亮：当前节点为品牌蓝实底白字，关联节点为浅蓝底、2px 品牌边框与外环，高亮箭头恢复通路色并增粗；所有未命中的节点、箭头和层级容器的背景、边框、文字、线色、透明度、滤镜与高亮前逐项一致，不得置灰或弱化；全部确认通路及草稿箭头头部均为 `8×8`；隐藏通路排除；通路聚焦和多选组合查询优先级正确；
6. Light/Dark 切换，主题 token 生效；
7. 1440×900 无页面级滚动，960×640 仍可查看，200% 缩放时提交按钮可达；
   - 增加“未命名通路图111”形态的 1920×1080 三栏工作台基准：中央有效画布约 1420×964，8 个层级（含父层）、7 个叶子层、42 个长短混合文本节点全部进入画布并保持每层单行；自动适应目标 125%，短节点显示约 80×40px、最长节点约 230×40px，14px 配置字体投影约 17.5px，同行节点左右留白对称；
...
- 新建模式支持画布点击加入、Shift+点击移除、Enter/Esc 和草稿/已确认视觉区分；选中已有通路即可通过 Shift+点击或 Shift+空格即时增删，普通点击节点保持活动通路，Esc 或退出按钮结束；
