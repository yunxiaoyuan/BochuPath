...
| 通路 | 新增、查看、编辑、删除、步骤轨道、按插入位置筛选合法候选、列表/画布点选、替换/移除、显示/隐藏、聚焦高亮 |
...
| Pathway | 通路名称、有序节点列表、颜色、线型、备注、是否显示 | 名称 1–80 字；至少 2 个且不重复的节点；相邻步骤必须按叶子层级视觉顺序严格向下（LR 时向右），允许跨层，禁止同层或回流 |
...
- 以层级树中叶子层级的深度优先视觉顺序作为唯一业务顺序；TB 的向下与 LR 的向右复用同一索引；
- 每个后续步骤的叶子层级索引必须严格大于前一步；允许跳过中间层级，禁止同层连接和向上/向左回流；
- 多条通路可共享节点和相邻节点对；通路仍是多节点有序序列，不要求覆盖全部层级；
- 删除通路只删除 Pathway 和 PathStep，不删除节点；
- 隐藏通路仅影响渲染，不删除数据；
- 新建/编辑通路、节点换层、层级移动或排序都必须复用该规则；若变更会破坏任一通路，则整条命令原子阻止并列出受影响通路；
- 编辑时通过首部、步骤之间或末尾的插入位置新增节点，通过相邻层级边界筛选替换候选；步骤移除实时预览，但点击“确定”前只存在于表单草稿；取消或 Esc 不留下半成品；
- V1 不提供任意步骤拖拽排序，因为任意交换会制造同层或回流；方向变化通过合法的插入、替换和移除完成。
...
编辑/连接
  ├─ 选择插入/替换位置 → 根据前后叶子层级边界生成候选集合
  ├─ 点击候选列表或画布合法节点 → 插入或替换草稿步骤
...
- 单选节点显示 2px 品牌描边；取该节点参与的全部可见通路，将这些通路的完整步骤节点和派生箭头作为关联并集：关联节点显示不覆盖业务样式的外环，关联箭头保留通路颜色并增粗，其余对象 dimmed；隐藏通路不因选择而显示；
...
- Pathway：`通路名称* → 步骤轨道* → 合法候选 → 颜色 → 线型 → 备注 → 是否显示`；每一步显示序号、节点名、完整层级路径、定位、替换和移除，并在首部、步骤之间、末尾提供插入入口；
...
```text
┌─ 工具栏：[连接通路]（品牌色激活）──────────────────────────────────────────┐
│ 提示：选择插入位置，再从列表或画布选择合法节点；Enter 完成，Esc 取消      │
├───────────────────────────────────────────────────────────────────────┤
│ [节点A ①] - - - -▶ [节点B ②] - - - -▶ [节点C ③] [✓ 完成通路]       │
│ 候选边为品牌色虚线；合法候选明显标记，非法节点弱化且不可点击           │
│                                        右侧步骤轨道与候选列表实时同步 │
└───────────────────────────────────────────────────────────────────────┘
```

- 进入后在工具栏下插入 36px 指引条，保持当前视口中心不跳动；
- 新建首节点可自由选择；之后仅允许选择叶子层级索引更大的下游节点；
- 已有通路在首部、步骤之间和末尾提供插入入口：首部只允许首步骤上游，中间只允许前后步骤之间，末尾只允许末步骤下游；替换使用同样的相邻边界；
- 候选按完整层级路径分组并按层级/节点顺序排列，支持名称和层级路径搜索，排除已使用节点；候选列表与画布共用同一个选择器结果；
- 每个步骤提供定位、替换、移除，不提供任意拖拽排序；编号与候选箭头随草稿实时更新；
- 少于 2 个合法节点时“确定”禁用并显示还需选择的节点数；
- Enter 或“确定”提交；Esc 或“取消”清除草稿并回到选择工具；
- 候选边为品牌虚线，已确认边为实线；编辑现有通路时草稿替代原通路预览，确认只提交一条可撤销命令。
...
- 通路至少两步、重复节点拒绝、合法跨层通过、同层/回流拒绝、步骤归一化；
- 首部/中间/末尾插入及替换的候选区间，结构调整破坏通路时原子阻止；
- 节点关联上下文包含全部可见通路的完整步骤并集，同时排除隐藏通路；
- 删除节点后的重连与不足两步阻止；
...
- 步骤轨道插入/替换/移除、搜索和画布候选一致，Enter 提交、Esc 清空、取消不写入；
...
2. 编辑已有通路，从候选列表和画布在首部/中间/末尾插入、替换或移除节点，箭头与编号同步更新，Undo/Redo 正确；
...
9. 在 `allow-scripts allow-same-origin allow-popups` 的 PageDrop 等价 sandbox iframe 内覆盖合法跨层、同层/回流阻止、通路编辑、关联高亮、保存重开、版本冲突、应用内 Dialog、键盘、缩放和 200% 浏览器缩放，控制台无 sandbox、表单提交、资源路径、Cookie 或 JSON 写入错误。
...
- 通路至少两步、无重复节点，叶子层级索引严格递增；同层和回流被阻止；
- 自动布局稳定，相同输入得到相同坐标；失败时不白屏；
- 树、画布、Inspector 使用唯一 Selection 并保持同步；
- 查看模式没有能改变数据的控件、快捷键或拖拽行为；
- 连接模式支持步骤轨道、区间候选、列表/画布选择、插入/替换/移除、Enter、Esc 和候选/已确认视觉区分；
