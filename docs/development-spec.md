...
| 层级 | 树形查看、新增顶层/子层级、重命名、排序、移动、删除、影响检查；节点只属于叶子层级 |
| 节点 | 新增、查看、编辑、复制、删除、搜索、定位；字段见 4.3 |
...
createLayer / updateLayer / moveLayer / deleteLayerWithMigration
createNode / updateNode / duplicateNode / deleteNode / replacePathwayNode
...
1. **结构**：树行高 32。层级行依次为展开箭头、层级图标、名称、节点数；节点显示在所属叶子层级下。hover/selected 时显示拖拽手柄和更多菜单。顶部 SplitButton：新增层级、新增节点；
...
- 新建/编辑时 InspectorFooter 固定显示“取消｜确定”；纯查看详情不显示提交按钮；
