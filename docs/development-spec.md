...
- 单选节点显示主品牌描边和连接控制点；取该节点参与的全部可见通路，将这些通路的完整成员节点和派生箭头作为关联并集：关联节点临时覆盖为单层 2px 品牌蓝实线边框并使用约 10% 蓝色底色染色，不与原虚线/点线边框叠加，清除选择后恢复原样式；关联箭头保留通路颜色并增粗；未关联节点保持原业务样式和正常不透明度，未关联通路箭头可降低对比；隐藏通路不因选择而显示；
