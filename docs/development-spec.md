...
7. 生成节点锚点后再派生并路由连线，跨层使用柔和贝塞尔曲线和小箭头，箭头指向下一占用层节点；常态连线以细灰色、低对比呈现，聚焦或节点关联高亮时恢复通路颜色并增粗；
8. 计算完整包围盒并支持“适应画布”；
...
- 单选节点显示品牌蓝实底、白字、2px 深品牌描边和连接控制点；取该节点参与的全部可见通路，将这些通路的完整成员节点和派生箭头作为关联并集：关联节点临时覆盖为浅蓝底、2px 品牌蓝实线边框与外环，不与原虚线/点线边框叠加；关联箭头保留通路颜色并增粗，常态/未关联箭头可使用细灰色曲线降低对比；**未关联节点必须保持原色、原透明度、原饱和度和原业务样式，禁止添加 `opacity`、灰度/饱和度滤镜、覆盖色或任何其他弱化处理。此项为不可回退的产品硬约束。** 清除选择后恢复原样式；隐藏通路不因选择而显示；
...
4. 删除被引用样式前强制选择替代样式；
5. 编辑及查看模式单选节点时完整关联节点/箭头高亮：当前节点为品牌蓝实底白字，关联节点为浅蓝底、2px 品牌边框与外环；未关联节点的背景、边框、文字、透明度、滤镜与未高亮前逐项一致，不得置灰或弱化；常态箭头为细灰色贝塞尔曲线，高亮箭头恢复通路色并增粗；隐藏通路排除；通路聚焦和多选组合查询优先级正确；
