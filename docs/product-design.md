...
- **通路**：引用多个节点，并按画布固定节点顺序生成步骤；
- **布局配置**：控制层级间距、节点间距和字号；
- **渲染结果**：根据以上配置生成层级容器、节点和通路线。

其中，通路更接近“按固定画布顺序排列的节点集合”，连线是相邻步骤的渲染结果，而不是用户任意绘制的独立边。
...
- **V1 控制复杂度**：通路使用画布固定顺序生成的节点序列，不支持任意网络、环路和条件边。
...
| PathStep | ID、通路 ID、节点 ID、步骤序号 | 表示通路节点成员；步骤序号由画布固定顺序规范化 |
...
| 通路管理 | 通路 CRUD、画布直接增删节点、固定顺序自动编号、显示隐藏、高亮 | P0 |
...
- 一条通路至少包含两个节点；
- 画布中的固定节点顺序决定通路方向，但“通路”不是把所有节点从头串到尾；它只连接用户选入该通路的业务节点；
- 叶子层级按层级树的深度优先视觉顺序编号，同层节点按 `order,id` 排列；通路可同时包含同一层的多个节点，也可跨层；TB/LR 共用同一固定业务顺序；
- 同一节点在一条通路中最多出现一次，V1 不支持环路；
- 多条通路可以共享节点；
- 删除通路不删除节点；
- 删除被引用节点前展示受影响通路，并明确相邻步骤的重连结果；
- 删除节点后若某条通路仍有至少两个步骤，则自动连接相邻步骤；若不足两个步骤，则阻止删除，并要求先删除该通路或指定替代节点；
- 节点换层、层级移动或排序后，所有受影响通路都按新的画布固定顺序自动重排；
...
- 节点所属层级决定其画布区域；
- 通路步骤映射为相邻节点间的有向连接；
- V1 采用自动布局，以层级顺序和同层节点顺序作为布局与通路步骤的共同输入；
...
- 普通表单底部固定“取消｜确定”，危险操作放入面板底部独立区域；连接通路草稿的完成和取消放在画布顶部指引条；
- 删除层级、节点或样式前展示依赖影响并支持撤销。

#### 通路配置

- 进入连接模式后，顶部按新建/编辑显示点击语义，并显示已选数量、“完成”和“取消”；
- 画布上的候选边使用品牌色虚线，已确认通路使用实线；
- 右侧只读展示当前步骤摘要，通路节点增删不在 Inspector 中放置控件；
- 新建时，普通点击未选节点即加入，`Shift+点击` 已选节点即移除；
- 编辑时，普通点击仍是选择节点，`Shift+点击` 切换该节点是否属于通路；
- 加入顺序不作为步骤顺序；系统自动按层级与同层节点的画布固定顺序重排，允许同层多节点；
- 至少两个合法节点后允许 Enter 或“确定”提交，整个草稿只生成一条可撤销命令；
...
关键架构决策：`Pathway + PathStep 节点成员` 是配置事实；PathStep 的 `order` 按画布固定节点顺序规范化，Edge 只在布局/渲染时产生。这样可避免通路步骤、画布节点顺序与派生连线产生冲突。
...
10. 通路可包含同层多节点并按画布固定顺序连接，新建时可点击加入，编辑时可通过 Shift+点击直接增删；
