...
- **通路**：引用多个节点，按画布固定叶子层级顺序形成分层有向图；
- **布局配置**：控制层级间距、节点间距和字号；
- **渲染结果**：根据以上配置生成层级容器、节点和通路线。

其中，通路是节点成员集合。系统按固定叶子层级顺序对成员分组，并在相邻占用层之间自动生成全连接边；连线是渲染结果，而不是用户任意绘制的独立边。
...
- **V1 控制复杂度**：通路只允许由相邻占用层自动全连接形成的分支与汇聚，不支持逐边选择、反向边、环路和条件边。
...
V1.0 优先实现编辑闭环；PageDrop 部署支持基于共享 JSON 与 revision 冲突检测的多人异步协作。V1.1 采用企业微信统一登录、内部成员默认只读，并通过邀请、申请审批或管理员直接开通获得写权限；权限安全边界和实施契约见 [`access-permission-design.md`](access-permission-design.md)。多人实时同屏协作继续后置。
...
| Pathway | ID、名称、节点 ID 集合、颜色、线型、显示状态、排序 | 表示一条至少占用两个叶子层级的分层有向图通路 |
...
- 一条 Pathway 的 `nodeIds[]` 至少覆盖两个不同叶子 Layer；数组仅用于确定性序列化，不表达链式顺序；
- 同一 Node 可被多条 Pathway 引用，但在同一 Pathway 中不得重复；
- 相邻占用层之间的笛卡尔积 Edge 是渲染结果，不是独立编辑或持久化对象。
...
| 通路管理 | 通路 CRUD、选中后画布直接增删节点、分层图自动派生、显示隐藏、高亮 | P0 |
| 画布呈现 | 层级容器、节点、方向连线、自动布局、缩放、平移、适应画布 | P0 |
| 查看模式 | 搜索、筛选、定位、节点详情、单节点全部可见关联通路高亮、单通路聚焦与多节点组合查询 | P0 |
...
- 一条通路至少占用两个不同的叶子层级；
- 画布中的固定叶子层级顺序决定通路方向；通路是用户选入节点组成的分层有向图，不是从头串到尾的节点链；
- 叶子层级按层级树的深度优先视觉顺序排列，同层成员按 `order,id` 确定性序列化；通路可包含同层多个节点并跳过未占用层级，TB/LR 共用同一业务方向；
- 相邻占用层使用笛卡尔积全连接，同层节点不连边，因此系统可自动形成分支与汇聚，但不允许只保留部分边；
- 同一节点在一条通路中最多出现一次，V1 不支持环路；
- 多条通路可以共享节点；
- 删除通路不删除节点；
- 删除被引用节点前展示受影响通路，并明确删除前后的占用层与派生边变化；
- 删除节点后若某条通路仍占用至少两个层级，则在新的相邻占用层之间重新派生全连接边；若只剩一个占用层，则阻止删除并要求先调整或删除通路；
- 节点换层、层级移动或排序后，所有受影响通路都按新的固定层级顺序重新分组并派生边；
- 通路可单独显示、隐藏或高亮；隐藏通路不会因节点选中而临时显示。

#### 画布

- 层级树映射为嵌套容器或泳道；
- 节点所属层级决定其画布区域；
- 通路节点集合按占用层分组，相邻占用层映射为全连接有向边；
- V1 采用自动布局，以层级顺序和同层节点顺序作为布局与通路确定性序列化的共同输入；
- V1 不开放完全自由拖拽坐标，手动微调和位置锁定进入 V1.1。

### 3.8 关键用户流程

`新建通路图 → 建立层级树 → 定义节点样式 → 创建并归属节点 → 创建通路并选择成员节点 → 自动生成分层图 → 校验 → 保存 → 切换查看模式验收`
...
- 单选节点后，高亮它参与的全部可见通路所包含的完整节点和箭头并集：当前节点使用品牌蓝实底和白字；关联节点临时使用浅蓝底、2px 品牌蓝实线边框与外环，不与原业务虚线/点线边框叠加；关联箭头保留通路颜色并增粗；高亮只允许增强命中对象，所有未命中的节点、箭头、层级、泳道和其他画布元素必须与高亮前保持完全相同，禁止置灰、降低透明度或以其他方式弱化；清除选择后恢复原样式；
- 右侧使用非模态只读详情抽屉；
- 支持单条通路聚焦、多个节点组合查询、适应画布和缩放。
- 单通路聚焦优先于节点关联高亮；Ctrl/Cmd 多选仍用于“包含全部所选节点”的组合查询；右侧详情分别显示可见与隐藏通路数量，并以状态文字播报高亮结果。

#### 编辑模式

- 左侧树、画布、右侧属性保持同一选择态；
- 工具条包含选择、框选、平移、新增节点、新建通路、可读布局、缩放与适应画布；“适应”完整显示整图，并在空间足够时自动放大至 125% 以提高文字可读性；“布局”保证约 12px 正文、10px 说明的可读下限并允许平移查看超出部分；侧栏开合、窗口缩放后保持用户最后选择的适配策略；
- 层级支持沿布局主轴拖动调整同级顺序，节点支持在所属叶子泳道内拖动调整同层顺序；拖动采用手机桌面图标式浮起、实时让位和松手吸附，不叠加吸附框或目标序号；结果保存为结构化 `order`，而非自由坐标，因此新增对象和重新布局不会覆盖既有调整；
- 画布依据实际可见区域自适应节点宽度、间距和换行容量，适应时四周保留 12px 安全边距；TB 节点先按文字取得 64–240px 自然宽度，空间不足时按比例收紧，64px 下限仍无法容纳才换行；无拆解内容节点以 32px 为紧凑高度，换行或拆解内容只增高自身。泳道按最宽一层的实际内容取宽并居中，不用空白强行撑满屏幕；LR 节点同样独立取宽高、每列垂直居中；
- 顶部工具栏已经提供缩放与适应，因此画布内不重复放置浮动缩放控件；导航缩略图默认关闭，用户主动打开时才允许覆盖右下角画布区域；
- 新增/重命名优先行内完成，复杂属性进入右侧面板；
- 结构面板提供批量添加：用中文/英文分号或换行分隔名称，同批层级共享上级、同批节点共享叶子层级与样式；添加节点时默认使用当前选中的叶子层级，选中节点时使用其所属层级，无上下文时回退到第一个叶子层级；添加过程中切换选中叶子层级时同步更新所属层级；预览确认后一次提交并可整体撤销；
- 普通表单底部固定“取消｜确定”，危险操作放入面板底部独立区域；新建通路草稿的完成和取消放在画布顶部指引条；
- 删除层级、节点或样式前展示依赖影响并支持撤销。

#### 通路配置

- 点击“新建通路”后顶部显示新建点击语义、节点数、层数、“完成”和“取消”；
- 画布上的候选边使用品牌色虚线，已确认通路支持实线、虚线和点划线；
- 右侧按占用层分组展示节点摘要、节点数、层数和派生边数，通路节点增删不在 Inspector 中放置控件；
- 新建时，普通点击未选节点即加入，`Shift+点击` 已选节点即移除；
- 点击通路列表项或通路边即把该通路设为独立的活动通路，左侧行和画布提示条持续标识当前通路，不提供“在画布编辑节点”二次按钮；
- 编辑已有通路时，`Shift+点击` 或聚焦节点后 `Shift+空格` 切换该节点是否属于通路并生成可撤销命令；普通点击节点只查看属性，不退出活动通路；
- 加入顺序不表达链式顺序；系统按固定叶子层级顺序分组，相邻占用层全连接；
- 至少占用两个层级后允许 Enter 或“确定”提交新建草稿；已有通路成员修改立即生效，属性表单只提交元数据；
- 点击空白只清除普通对象选择；Esc、活动提示条退出按钮、选择层级/样式或切换模式退出已有通路编辑；退出新建模式时清理临时候选，不保留半成品。
...
- **规则校验器**：层级无环、叶子归属、样式引用、通路占用层与删除影响；
- **布局与渲染引擎**：由 Diagram Schema 生成层级容器、节点和 Edge；
- **持久化服务**：保存、读取、版本字段和 Schema 迁移；
- **交换层**：导入导出和分享链接。

关键架构决策：`Pathway.nodeIds[]` 是通路唯一配置事实；数组仅按画布固定顺序确定性序列化，不表示节点链。Edge 由相邻占用层的节点笛卡尔积在布局/渲染时产生，不独立持久化，从而避免成员集合、层级顺序与派生连线冲突。Diagram Schema `1.1` 通过显式迁移提取旧 `steps[].nodeId`；无法满足至少两层约束的旧数据停止迁移并报告。

PageDrop 运行环境以 `allow-scripts allow-same-origin allow-popups` 的 sandbox iframe 为产品约束：Vite 仅在本地构建，线上运行相对资源路径的静态包和 Hash 路由；正式数据通过同外链 JSON SDK/相对请求读写。由于没有 `allow-forms`、`allow-modals` 和 `allow-downloads`，所有提交、确认和错误反馈使用 React 事件与应用内组件，不依赖原生表单导航、浏览器模态框或 Blob 下载。图库返回和模式切换使用应用内脏状态拦截；关闭父页面不能可靠拦截时，以 500ms 个人草稿和再次进入的恢复提示防丢失。V1 不引入 Worker、Service Worker、父窗口 DOM 操作、顶层跳转或跨域接口。
...
8. 单条通路高亮时，占用层、成员节点和方向清晰可辨；
9. 单选节点时，编辑和查看模式都能高亮其全部可见通路的完整节点和箭头；
10. 通路可包含同层多节点并跳层，相邻占用层自动全连接；新建时可点击加入，选中已有通路后可通过 Shift+点击直接增删；
11. 撤销重做覆盖层级、节点、样式和通路的核心编辑命令；
12. 1440×900 下主要工作区无需页面级滚动，面板内部可独立滚动。
...
| 风险 | 影响 | 应对 |
| --- | --- | --- |
| 用户需要逐边控制分支/汇聚 | 当前自动全连接表达力不足 | 保持 V1 分层有向图约束；选择性边进入后续版本 |
