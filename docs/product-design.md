...
- 单选节点后，高亮它参与的全部可见通路所包含的完整节点和箭头并集；关联节点临时使用单层 2px 品牌蓝实线边框和轻微蓝色底色染色，不与原业务虚线/点线边框叠加，清除选择后恢复原样式；关联箭头保留通路颜色并增粗；未关联节点保持原业务样式和正常不透明度，仅未关联通路箭头可降低对比；
- 右侧使用非模态只读详情抽屉；
