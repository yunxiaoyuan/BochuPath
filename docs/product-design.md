...
- 单选节点后，高亮它参与的全部可见通路所包含的完整节点和箭头并集：当前节点使用品牌蓝实底和白字；关联节点临时使用浅蓝底、2px 品牌蓝实线边框与外环，不与原业务虚线/点线边框叠加；关联箭头保留通路颜色并增粗；未关联节点必须保持原色、原透明度、原饱和度和原业务样式，禁止置灰或以任何方式弱化；常态箭头可使用细灰色曲线退后；清除选择后恢复原样式；
- 右侧使用非模态只读详情抽屉；
