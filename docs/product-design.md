...
| 通路管理 | 通路 CRUD、步骤轨道、插入位置、合法候选、列表/画布选择、替换/移除、显示隐藏、高亮 | P0 |
...
- 一条通路至少包含两个节点；
- 节点顺序决定通路方向，但“通路”不是把所有节点从头串到尾；它只表达所选业务节点从上一叶子层级指向下一叶子层级的关系；
- 叶子层级按层级树的深度优先视觉顺序编号，通路每个后续步骤的层级索引必须严格增大；允许跨层，禁止同层连接和向上回流；TB 的向下与 LR 的向右使用同一业务顺序；
- 同一节点在一条通路中最多出现一次，V1 不支持环路；
- 多条通路可以共享节点；
- 删除通路不删除节点；
- 删除被引用节点前展示受影响通路，并明确相邻步骤的重连结果；
- 删除节点后若某条通路仍有至少两个步骤，则自动连接相邻步骤；若不足两个步骤，则阻止删除，并要求先删除该通路或指定替代节点；
- 节点换层、层级移动或排序若会破坏通路的层级递增规则，则整次修改被阻止并列出受影响通路；
...
- 进入连接模式后，顶部显示“选择插入位置，再从列表或画布选择合法节点；Enter 完成，Esc 取消”；
- 画布上的候选边使用品牌色虚线，已确认通路使用实线；
- 右侧使用步骤轨道，首部、步骤之间和末尾都有插入入口；每一步展示序号、节点名、所属层级、定位、替换和移除；
- 激活插入或替换位置后，根据前后步骤层级边界生成候选：首部仅上游，中间仅边界之间，末尾仅下游；新建首节点可自由选择；
- 候选按完整层级路径分组和排序，支持名称/路径搜索并排除通路已有节点；列表与画布共用候选集合，合法节点明显标记，非法节点弱化且不可点击；
- 不提供任意步骤拖拽排序；画布点选与步骤轨道双向联动，合法变化实时更新箭头和编号；
...
10. 通路只允许沿叶子层级顺序向下/向右，可通过步骤轨道从列表或画布高效插入、替换和移除；
