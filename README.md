...
一个由结构化 `Diagram` 数据驱动的业务通路图桌面 Web 工具。用户维护层级、节点、语义节点样式和有序通路；画布坐标与连线均自动派生，不保存自由坐标或独立 Edge。通路表示从上一叶子层级指向下一叶子层级的业务关系，不要求串联图中全部节点。
...
- `src/domain`：V1.0 持久化类型、Zod Schema、稳定错误码、叶子层级业务顺序、不变量、合法候选选择器与种子数据。
...
画布的“布局”和“适应”以实际可见画布尺寸和布局边界计算缩放，不会把三栏工作台外框误当作可用区域；在 16:9 等宽屏中优先让同层节点保持单行，通过缩窄节点、按文字换行增高、压缩间距提高整图字号的屏幕可读性，只有单行缩放低于可读性下限时才自动换行。宽屏顶栏可随时收起对象面板或属性面板，为密集图释放更多空间。确认通路和连接草稿都显示方向箭头及步骤序号。层级树、节点/通路列表、画布和 Inspector 共用同一个选择状态，并支持鼠标与 Enter/Space 键操作。

通路的业务顺序取层级树中叶子层级的深度优先视觉顺序。每个后续步骤必须位于更下游的叶子层级；允许跨层，但同层连接和向上回流会以 `PATHWAY_LAYER_ORDER_INVALID` 原子阻止。TB 的向下和 LR 的向右复用同一业务顺序。节点换层、层级移动或排序也会在提交前检查所有受影响通路，失败提示包含通路名称。

新建和编辑通路使用“步骤轨道”：首部、步骤之间和末尾均可选择插入位置，每一步可以定位、替换或移除。候选由前后步骤的层级区间计算，按完整层级路径分组并支持搜索；候选列表和画布共用同一集合，非法节点弱化且不可点击。取消或 Esc 不写入 `Diagram`，确定或 Enter 只提交一条可撤销领域命令，不支持任意步骤拖拽排序。

在编辑或查看模式单选节点时，应用会高亮该节点参与的全部可见通路，以及这些通路的完整步骤节点和派生箭头；隐藏通路保持隐藏，详情区分别显示可见/隐藏数量。单通路聚焦优先，Ctrl/Cmd 多选仍用于查询同时包含全部所选节点的通路。
...
Playwright 使用与线上相同的完整 sandbox 字符串回归图库/Inspector CRUD、合法跨层和同层/回流阻止、从列表与画布编辑通路、编辑/查看关联高亮、共享保存、刷新重开、草稿恢复、版本冲突、应用内 Dialog、键盘、拖动、缩放和 200% 浏览器缩放，并检查页面控制台错误。真实已发布外链的写入测试需要单独配置 PageDrop API Token，本仓库测试不会修改线上外链。

## V1.0 边界

不支持独立 NodeType、独立持久化 Edge、条件分支/汇聚/环路、同一通路重复节点、同层或向上回流、任意通路步骤拖拽排序、自由坐标定位/锁定、手工折线、多人实时同屏协作、自动合并、评论和导入导出。画布拖动仅调整层级或节点的结构化 `order`，且不能破坏已有通路；节点与连线仍是 `Diagram` 的确定性派生视图。
