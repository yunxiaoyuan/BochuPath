...
一个由结构化 `Diagram` 数据驱动的业务通路图桌面 Web 工具。用户维护层级、节点、语义节点样式和通路节点集合；画布坐标、通路步骤顺序与连线均自动派生，不保存自由坐标或独立 Edge。通路可连接同层或跨层的所选业务节点，不要求串联图中全部节点。
...
- `src/domain`：V1.0 持久化类型、Zod Schema、稳定错误码、叶子层级/同层节点固定顺序、不变量与种子数据。
...
画布的“布局”和“适应”以实际可见画布尺寸和布局边界计算缩放，不会把三栏工作台外框误当作可用区域；在 16:9 等宽屏中优先让同层节点保持单行，通过缩窄节点、按文字换行增高、压缩间距提高整图字号的屏幕可读性，只有单行缩放低于可读性下限时才自动换行。宽屏顶栏可随时收起对象面板或属性面板，为密集图释放更多空间。确认通路和连接草稿都显示方向箭头及步骤序号。层级树、节点/通路列表、画布和 Inspector 共用同一个选择状态，并支持鼠标与键盘操作。

通路的业务顺序由画布固定：先取层级树中叶子层级的深度优先顺序，再取同层节点的 `order,id` 顺序。一条通路可包含同一层的多个节点，也可跨层；加入节点的先后不改变通路顺序。节点换层、层级移动或节点/层级排序后，受影响通路会自动按新的画布顺序重排。

通路节点直接在画布上编辑。新建时普通点击未选节点加入，`Shift+点击` 已选节点移除；编辑已有通路时，普通点击仍用于选择节点，`Shift+点击` 切换该节点是否属于通路。Inspector 只显示步骤摘要和通路属性，不提供插入位置或节点旁操作按钮。取消或 Esc 不写入 `Diagram`，确定或 Enter 只提交一条可撤销领域命令。
...
不支持独立 NodeType、独立持久化 Edge、条件分支/汇聚/环路、同一通路重复节点、任意通路步骤拖拽排序、自由坐标定位/锁定、手工折线、多人实时同屏协作、自动合并、评论和导入导出。画布拖动仅调整层级或节点的结构化 `order`；受影响通路会随新顺序自动重排，节点与连线仍是 `Diagram` 的确定性派生视图。
